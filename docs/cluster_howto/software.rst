...
Commonly used data analysis/process software are centrally managed and supported in the cluster. A list of the supported software can be found `here <http://intranet.donders.ru.nl/index.php?id=966>`_. The repository where the software are organised and installed is mounted to the ``/opt`` directory on every cluster node.
